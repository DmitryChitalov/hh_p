...
"""
     ...
     для поиска process_requestвакансий по указанной тематики и обрабатывает 
...
class HHRequest:
    ...
    def set_region(self, name):
        j_params = {'text': name}
        print(1, j_params)
        j_result = req.get('https://api.hh.ru/suggests/areas',
                           params=j_params).json()
        print(2, j_result)
        if j_result["items"]:
    ...
    def get_urls_vacancies(self):
        ...
        while True:
            ...
            if j_result["page"] >= j_result["pages"] - 1:
                ...
                i_page_number += 1
        #print(f"вах {self._l_urls_vacancies}")
